...
while True:
    try:
        ...
        if wans and ports and mdm_present:
            ...
            if is_available_wan == 0 and not any(
                x in model for x in MODELS_WITHOUT_WAN
            ):
                ports_status += "WAN: ⚫️ "
            ...
            for port in ports:
                ...
                if (
                    port["port"] == 0 and any(x in model for x in MODELS_WITHOUT_WAN)
                ) or (port["port"] >= 1):
                    if port["link"] == "up":
                        ports_status += " 🟢 "
                    else:
            ...
            for wan in (wan for wan in wans if "mdm" in wan):
                ...
                if "mdm" in wan:
                    ...
                    summary = cp.get(
                        "/status/wan/devices/{}/status/summary".format(wan)
                    )
                    ...
                    if summary:
                        ...
                        if "connected" in summary:
                            ...
                            or "standby" in summary
                            or "suspended" in summary
                            or "connecting" in summary
                            or "transitioning" in summary
                            or "unready" in summary
                            or "unconfigured" in summary
                            or "operation failed" in summary
                            or "switch" in summary
                        ):
