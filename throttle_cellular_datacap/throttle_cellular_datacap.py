...
class DataUsageCheck(object):
    ...
    def set_throttle(self, modem_profiles_list):
        for mdm in modem_profiles_list:
            self.cp.put(
                self.CFG_RULES2_PATH + "/" + mdm + "/bandwidth_egress", self.minbwup
            )
            self.cp.put(
                self.CFG_RULES2_PATH + "/" + mdm + "/bandwidth_ingress", self.minbwdown
            )
        self.cp.put("config/qos/enabled", True)
        self.cp.log("Exceeded monthly data usage threshold - reducing LTE data rate")
        message = (
