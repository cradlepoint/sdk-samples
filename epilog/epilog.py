"""epilog - SDK Application that writes log to files on flash available for download via HTTP/Remote Connect.

Never lose another log!  Remote Syslog!
No more log rolling over, no more physical USB flash drives,
and you can recover log after a reboot.  Via Remote Connect!

A "Full Log {MAC ADDRESS}.txt" file will be created containing all log.
Individual "Log {MAC ADDRESS} {TIMESTAMP}.txt" files will be created at each boot for isolating log.
