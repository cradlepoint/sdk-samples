...
class SIMSpeedTest(object):
    ...
    ONLY_RUN_ONCE = (
        False  # True means do not run if Boot2 has been run on this device before.
    )
    ...
    def check_if_run_before(self):
        if self.ONLY_RUN_ONCE:
            if (
                self.client.get("/config/wan/rules2/0/_id_")
    ...
    def test_sim(self, device):
        try:
            if self.modem_state(device, "connected"):
                ...
                (
                    self.sims[device]["download"],
                    self.sims[device]["upload"],
                ) = self.do_speedtest(self.sims[device])
                self.client.log(
                ...
                if (
                    self.sims[device].get("download", 0.0) > self.MIN_DOWNLOAD_SPD
                    and self.sims[device].get("upload", 0.0) > self.MIN_UPLOAD_SPD
                ):
                    return True
    ...
    def create_message(self, uid, *args):
        ...
        for arg in args:
            if arg == "download":
                message = (
                    "DL:{:.2f}Mbps".format(self.sims[uid]["download"])
                    if not message
                    else " ".join(
                ...
                message = (
                    "UL:{:.2f}Mbps".format(self.sims[uid]["upload"])
                    if not message
                    else " ".join(
                ...
                "HOMECARRID",
                "RFBAND",
            ]:  # Do not include labels for these fields
                message = (
                    "{}".format(self.sims[uid]["diagnostics"][arg])
                    if not message
                    else " ".join(
                ...
                message = (
                    "{}:{}".format(arg, self.sims[uid]["diagnostics"][arg])
                    if not message
                    else " ".join(
                        [
                            message,
    ...
    def run(self):  # *** Main Application Starts Here ***
        ...
        # Send startup alert
        message = (
            f"Boot2 Starting! {system_id} - {product_name} - Router ID: {router_id}"
        )
        ...
        success = (
            False  # Boot2 Success Status - Becomes True when a SIM meets minimum speeds
        )
        ...
        sorted_results = sorted(
            self.sims, key=lambda x: self.sims[x]["download"], reverse=True
        )
        ...
        for (
            uid
        ) in sorted_results:  # Add the results of each SIM with the fields specified:
            results_text = " | ".join(
                [
                    results_text,
                    self.create_message(
                        uid, "PRD", "HOMECARRID", "RFBAND", "RSRP", "download", "upload"
                    ),
                ]
            )
        ...
        if apikeys.get("X-ECM-API-ID") != "YOUR":
            ...
            req = requests.put(
                f"{self.API_URL}/routers/{router_id}/",
                headers=apikeys,
