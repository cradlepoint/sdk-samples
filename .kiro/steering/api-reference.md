...
- **ALWAYS include timestamps when reading logs** - log entries have Unix epoch timestamp at index 0
- **Convert timestamps to human-readable** - use `datetime.fromtimestamp(ts)` to compare against deploy time
- **Filter by recency** - after deploying, only look at logs with timestamps AFTER the deploy started, not old cached entries
