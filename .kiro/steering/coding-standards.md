...
- **Rate-limit config writes** - rapid `cp.put()` to `config/` paths causes excessive eventing load on the config store and floods NCM event triggers with data. Use a cooldown timer (e.g. 30-60s, or only write on actual value change) to prevent rapid-fire writes
