        ...
        for file in files:
            ...
            try:
                if expected_checksum[manifest_file_name] != checksum:
                    raise InvalidHash(
                        'File "{}" has been modified'.format(manifest_file_name)
                    )
                del expected_checksum[manifest_file_name]
            except KeyError:
                raise InvalidHash('File "{}" has been added'.format(manifest_file_name))

    if len(expected_checksum) != 0:
        raise InvalidHash(
            "Files have been removed: {}".format(list(expected_checksum.keys()))
        )
...
def validate_signature(app_root, cert):
    ...
    with open(signature_file, "rb") as sf:
        signature = sf.read()
        checksum = file_checksum(hashlib.sha256, file=app_manifest_file).encode("utf-8")
