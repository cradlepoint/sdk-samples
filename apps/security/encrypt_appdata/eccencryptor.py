...
if __name__ == "__main__":
    """ Use this tool to encrypt or decrypt JSON data using ECC encryption.
        Example usage:
    ...
    parser = argparse.ArgumentParser(description="Encrypt or decrypt JSON data using ECC")
    parser.add_argument("data", nargs="?", help="JSON data to encrypt/decrypt (omit to read from stdin)")
    args = parser.parse_args()
    
    # Initialize encryptor
    encryptor = ECCEncryptor()
    ...
    # Determine if the input is encrypted
    if encryptor.is_encrypted(input_data):
        # Decrypt
        try:
            decrypted_json = encryptor.decrypt_json(input_data)
            print(json.dumps(decrypted_json))
        except Exception as e:
            print(f"Error decrypting data: {e}", file=sys.stderr)
            sys.exit(1)
    else:
        # Try to parse as JSON and encrypt
        try:
            # Check if input is JSON string or literal JSON
            try:
                json_data = json.loads(input_data)
            except:
                # If not valid JSON, try treating as a string
                json_data = input_data
            
            encrypted_text = encryptor.encrypt_json(json_data)
