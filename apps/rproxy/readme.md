...
Configure `config/system/sdk/appdata` with entries for each port you want to proxy and the target IP/port in the form `local_addr:local_port:remote_addr:remote_port:proto`.
...
For dynamic clients, add a client entry to `config/system/sdk/appdata` with `rproxy.auto` in the form `local_host:local_port_ranges:remote_port:protocol`. Each client that connects on the LAN network will get a port in the range specified and traffic will be forwarded to the remote port.
...
| `rproxy.auto.2` | `80,8000-8010:80:tcp` | Ranges can be specified with commas |
