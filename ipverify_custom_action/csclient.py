...
class CSClient(object):
        ...
        if "linux" in sys.platform:
            ...
            try:
                response = requests.get(
                    device_api, auth=self._get_auth(device_ip, username, password)
                )
    ...
    def _get_auth(self, device_ip, username, password):
        ...
        try:
            response = requests.get(
                device_api, auth=requests.auth.HTTPBasicAuth(username, password)
            )
    ...
    def _get_device_access_info():
        ...
        if "linux" not in sys.platform:
            ...
            settings_file = os.path.join(
                os.path.dirname(os.getcwd()), "sdk_settings.ini"
            )
            ...
            if sdk_key in config:
                ...
                if username_key in config[sdk_key]:
                    ...
                    print(
                        "ERROR 2: The {} key does not exist in {}".format(
                ...
                print(
                    "ERROR 4: The {} section does not exist in {}".format(
    ...
    def _dispatch(self, cmd):
        ...
        try:
            ...
            errmsg = "dispatch failed with exception={} err={}".format(
                type(err), str(err)
            )
    ...
    def _safe_receive(self, sock):
        ...
        while remaining > 0:
            ...
            )  # TODO: This will hang things as well.
            if len(buf) == 0:
                break
            data += buf
            remaining -= len(buf)
    ...
    def _receive(self, sock):
        ...
        try:
            ...
            # ignore the command error, continue on to next command
            errmsg = "_receive failed with exception={} err={}".format(
                type(err), str(err)
            )
        if errmsg is not None:
            self.log(self.app_name, errmsg)
...
class EventingCSClient(CSClient):
    ...
    def _handle_events(self):
        poller = select.poll()
        poller.register(
            self.event_sock, select.POLLIN | select.POLLERR | select.POLLHUP
        )  # I don't unregsiter this in cleaning up!
        while self.running:
            try:
                        ...
                        try:
                            ...
                            try:
                                ...
                                if traceback:
                                    traceback.print_exc()
                                self.log(
                                    f"Exception during callback for {str(self.registry[eid])}"
                                )
                            if (
                                result["data"]["action"] == "get"
                            ):  # We've something to send back.
                                # config_store_receiver expects json
                                cb_return = json.JSONEncoder().encode(cb_return)
                                conn.sendall(
                                    cb_return.encode()
                                )  # No dispatch. Config store receiver will put to config store.
    ...
    def register(
        self, action: object, path: object, callback: object, *args: object
    ) -> object:
        if not self.running:
            self.start()
