...
class _Base(object):
    ...
    def override_user(
        ...
        for a specific user.
        """
        if not password and not homedir and not perm and not msg_login and not msg_quit:
            ...
            if require_valid_shell:
                for username in self.allowed_users:
                    if not self._has_valid_shell(username):
                        raise AuthorizerError(
                            "user %s has not a valid shell_sample" % username
                        )
...
try:
    ...
    class BaseWindowsAuthorizer(object):
        ...
        def __init__(self, anonymous_user=None, anonymous_password=None):
            ...
            if self.anonymous_user is not None:
                self.impersonate_user(self.anonymous_user, self.anonymous_password)
