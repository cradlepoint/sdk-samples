...
def main():
    ...
    parser = optparse.OptionParser(
        usage=usage, description=main.__doc__, formatter=CustomizedOptionFormatter()
    )
    parser.add_option(
        "-i",
        "--interface",
    ...
    parser.add_option(
        ...
        action="store_true",
        default=False,
        help="grants write access for the anonymous user " "(default read-only)",
    )
    parser.add_option(
        "-d",
        "--directory",
    ...
    parser.add_option(
        "-D", "--debug", action="store_true", help="enable DEBUG logging evel"
    )
    parser.add_option(
        "-v", "--version", action="store_true", help="print pyftpdlib version and exit"
    )
