...
class TestFtpDummyCmds(unittest.TestCase):
    ...
    def test_opts_feat(self):
        ...
        self.assertTrue("*" not in mlst())
        self.assertEqual(
            self.client.sendcmd("opts mlst fish;cakes;type;"), "200 MLST OPTS type;"
        )
        self.assertTrue("type*;perm;size;modify;" in mlst())
...
class TestFtpCmdsSemantic(unittest.TestCase):
    ...
    def test_no_auth_cmds(self):
        ...
        self.assertRaisesRegex(
            ftplib.error_perm, "530 Log in with USER", self.client.sendcmd, "stat /"
        )
        self.client.sendcmd("quit")
...
class TestFtpFsOperations(unittest.TestCase):
    ...
    def test_rmd(self):
        ...
        self.assertRaisesRegex(
            ftplib.error_perm, "Can't remove root directory", self.client.rmd, u("/")
        )
    ...
    def test_rnfr_rnto(self):
        ...
        self.assertRaises(
            ftplib.error_perm, self.client.sendcmd, "rnto " + self.tempfile
        )
    ...
    if not hasattr(os, "chmod"):
        ...
        def test_site_chmod(self):
            # not enough args
            self.assertRaises(ftplib.error_perm, self.client.sendcmd, "site chmod 777")
            # bad args
            self.assertRaises(
                ftplib.error_perm,
                self.client.sendcmd,
...
class TestFtpStoreData(unittest.TestCase):
    ...
    def test_appe_rest(self):
        ...
        self.client.sendcmd("rest 10")
        self.assertRaisesRegex(
            ftplib.error_temp, "Can't APPE while REST", self.client.sendcmd, "appe x"
        )
    ...
    def test_rest_on_stor(self):
        ...
        self.client.voidcmd("TYPE I")
        with contextlib.closing(self.client.transfercmd("stor " + TESTFN)) as conn:
            bytes_sent = 0
            while True:
    ...
    def test_failing_rest_on_stor(self):
        ...
        self.client.sendcmd("rest 10")
        self.assertRaises(
            ftplib.error_perm, self.client.storbinary, "stor " + TESTFN, lambda x: x
        )
...
@unittest.skipIf(
    sys.version_info < (3, 3) and sendfile is None, "pysendfile not installed"
)
...
class TestFtpRetrieveData(unittest.TestCase):
    ...
    def test_retr(self):
        ...
        bogus = os.path.basename(tempfile.mktemp(dir=HOME))
        self.assertRaises(
            ftplib.error_perm, self.client.retrbinary, "retr " + bogus, lambda x: x
        )
    ...
    def test_restore_on_retr(self):
        ...
        received_bytes = 0
        self.client.voidcmd("TYPE I")
        with contextlib.closing(self.client.transfercmd("retr " + TESTFN)) as conn:
            conn.settimeout(TIMEOUT)
            while True:
                chunk = conn.recv(BUFSIZE)
                if not chunk:
...
class TestFtpListingCmds(unittest.TestCase):
    ...
    def _test_listing_cmds(self, cmd):
        ...
        bogus = os.path.basename(tempfile.mktemp(dir=HOME))
        self.assertRaises(
            ftplib.error_perm, self.client.retrlines, "%s " % cmd + bogus, lambda x: x
        )
...
class TestFtpAbort(unittest.TestCase):
    ...
    @unittest.skipIf(sys.version_info < (2, 6), "does not work on python < 2.6")
...
class TestTimeouts(unittest.TestCase):
    ...
    def _setUp(
        self, idle_timeout=300, data_timeout=300, pasv_timeout=30, port_timeout=30
    ):
        self.server = self.server_class()
        with self.server.lock:
    ...
    def test_data_timeout_not_reached(self):
        ...
        self._setUp(data_timeout=0.1)
        with contextlib.closing(self.client.transfercmd("stor " + TESTFN)) as sock:
            if hasattr(self.client_class, "ssl_version"):
...
class TestConfigurableOptions(unittest.TestCase):
    ...
    def test_max_connections(self):
        ...
        try:
            ...
            self.assertRaises(
                ftplib.error_temp, c3.connect, self.server.host, self.server.port
            )
            ...
            c1.makepasv()
            self.assertRaises(
                ftplib.error_temp, c2.connect, self.server.host, self.server.port
            )
    ...
    def test_max_connections_per_ip(self):
        ...
        try:
            ...
            self.assertRaises(
                ftplib.error_temp, c4.connect, self.server.host, self.server.port
            )
            # Make sure client has been disconnected.
    ...
    def test_permit_privileged_ports(self):
        ...
        with self.server.lock:
            self.server.handler.permit_privileged_ports = False
        self.assertRaises(ftplib.error_perm, self.client.sendport, HOST, port)
        if sock:
    ...
    @unittest.skipUnless(hasattr(socket, "TCP_NODELAY"), "TCP_NODELAY not available")
...
class TestCallbacks(unittest.TestCase):
    ...
    def test_on_incomplete_file_sent(self):
        ...
        with contextlib.closing(
            self.client.transfercmd("retr " + TESTFN, None)
        ) as conn:
            while True:
                chunk = conn.recv(BUFSIZE)
                bytes_recv += len(chunk)
...
class _TestNetworkProtocols(object):
    ...
    def test_eprt(self):
        ...
        # port < 0
        self.assertEqual(self.cmdresp("eprt |%s|%s|-1|" % (self.proto, self.HOST)), msg)
        # port < 1024
...
class TestIPv6Environment(_TestNetworkProtocols, unittest.TestCase):
    ...
    def test_port_v6(self):
        ...
        self.assertRaises(
            ftplib.error_perm, self.client.sendport, self.server.host, self.server.port
        )
...
class TestUnicodePathNames(unittest.TestCase):
    ...
    def test_rnfr_rnto(self):
        if self.utf8fs:
            ...
            self.assertRaises(
                ftplib.error_perm, self.client.rename, TESTFN_UNICODE, TESTFN
            )
    ...
    def test_size(self):
        ...
        if self.utf8fs:
            ...
            self.assertRaises(
                ftplib.error_perm, self.client.sendcmd, "size " + TESTFN_UNICODE_2
            )
    ...
    def test_mdtm(self):
        if self.utf8fs:
            ...
            self.assertRaises(
                ftplib.error_perm, self.client.sendcmd, "mdtm " + TESTFN_UNICODE_2
            )
    ...
    def test_stou(self):
        if self.utf8fs:
            ...
            self.assertRaises(
                ftplib.error_perm, self.client.sendcmd, "stou " + TESTFN_UNICODE
            )
    ...
    def test_mlst(self):
        ...
        if self.utf8fs:
            self.assertTrue("type=dir" in mlstline("mlst " + TESTFN_UNICODE))
            self.assertTrue("/" + TESTFN_UNICODE in mlstline("mlst " + TESTFN_UNICODE))
            self.assertTrue("type=file" in mlstline("mlst " + TESTFN_UNICODE_2))
            self.assertTrue(
                "/" + TESTFN_UNICODE_2 in mlstline("mlst " + TESTFN_UNICODE_2)
            )
        else:
            self.assertRaises(ftplib.error_perm, mlstline, "mlst " + TESTFN_UNICODE)
