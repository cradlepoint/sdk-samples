...
class TestDummyAuthorizer(unittest.TestCase):
    ...
    def test_common_methods(self):
        ...
        self.assertRaises(
            AuthenticationFailed, auth.validate_authentication, USER, "wrongpwd", None
        )
        ...
        self.assertRaisesRegex(
            ValueError, "no such directory", auth.add_user, USER, PASSWD, "?:\\"
        )
        self.assertRaisesRegex(
            ValueError, "no such directory", auth.add_anonymous, "?:\\"
        )
        ...
        self.assertRaisesRegex(
            ValueError, "user 'anonymous' already exists", auth.add_anonymous, HOME
        )
        auth.remove_user(USER)
        auth.remove_user("anonymous")
        ...
        self.assertRaisesRegex(
            ValueError, "no such permission", auth.add_anonymous, HOME, perm="?"
        )
    ...
    def test_override_perm_interface(self):
        ...
        self.assertRaisesRegex(
            ValueError, "no such directory", auth.override_perm, USER, "?:\\", "elr"
        )
        self.assertRaisesRegex(
            ValueError,
            "no such directory",
        ...
        # raise on wrong permission
        self.assertRaisesRegex(
            ValueError, "no such permission", auth.override_perm, USER, HOME, perm="?"
        )
        # expect warning on write permissions assigned to anonymous user
        auth.add_anonymous(HOME)
        for p in "adfmw":
...
class _SharedAuthorizerTests(object):
    ...
    def test_impersonate_user(self):
        ...
        try:
            if self.authorizer_class.__name__ == "UnixAuthorizer":
                ...
                self.assertRaises(
                    AuthorizerError, auth.impersonate_user, nonexistent_user, "pwd"
                )
            else:
                self.assertRaises(
                    Win32ExtError, auth.impersonate_user, nonexistent_user, "pwd"
                )
                self.assertRaises(
                    Win32ExtError, auth.impersonate_user, self.get_current_user(), ""
                )
        finally:
    ...
    def test_override_user_password(self):
        ...
        self.assertRaises(
            AuthenticationFailed(auth.validate_authentication, user, "bar", None)
        )
...
@unittest.skipUnless(UnixAuthorizer is not None, "UnixAuthorizer class not available")
class TestUnixAuthorizer(_SharedAuthorizerTests, unittest.TestCase):
    ...
    def test_get_perms_anonymous(self):
        auth = UnixAuthorizer(global_perm="elr", anonymous_user=self.get_current_user())
    ...
    def test_validate_authentication(self):
        ...
        self.assertRaises(
            AuthenticationFailed, auth.validate_authentication, "?!foo", "?!foo", None
        )
    ...
    def test_validate_authentication_anonymous(self):
        current_user = self.get_current_user()
        auth = UnixAuthorizer(anonymous_user=current_user, require_valid_shell=False)
        self.assertRaises(
            AuthenticationFailed, auth.validate_authentication, "foo", "passwd", None
        )
        self.assertRaises(
            AuthenticationFailed,
            auth.validate_authentication,
    ...
    def test_not_root(self):
        ...
        try:
            ...
            self.assertRaisesWithMsg(
                AuthorizerError, "super user privileges are required", UnixAuthorizer
            )
        finally:
