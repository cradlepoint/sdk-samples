    ...
    try:
        response = requests.get(
            ...
            auth=requests.auth.HTTPBasicAuth(
                g_dev_client_username, g_dev_client_password
            ),
            verify=False,
        )
...
def get(config_tree):
    ...
    try:
        ...
        print(
            "Error with get for NCOS device at {}. Exception: {}".format(
                g_dev_client_ip, ex
            )
        )
        return None
...
def put(value):
    try:
        ...
        print(
            "Error with put for NCOS device at {}. Exception: {}".format(
                g_dev_client_ip, ex
            )
        )
        return None
        ...
        for fl in files:
            with open(os.path.join(root, fl), "rb") as f:
                if b"\r" in f.read() and [x for x in scanfiles if fl.endswith(x)]:
                    raise Exception(
                        "Carriage return (\\r) found in file %s"
...
def package():
    ...
    try:
        subprocess.check_output(
            "{} {} {}".format(g_python_cmd, package_script_path, app_path), shell=True
        )
...
def package_all():
    ...
    for app in app_dirs:
        ...
        try:
            ...
            subprocess.check_output(
                "{} {} {}".format(g_python_cmd, package_script_path, app_path),
                shell=True,
            )
        except subprocess.CalledProcessError as err:
...
def install():
    if is_NCOS_device_in_DEV_mode():
        ...
        cmd = "sshpass -p {0} scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no {1} {2}@{3}:/app_upload".format(
            g_dev_client_password, app_archive, g_dev_client_username, g_dev_client_ip
        )
        ...
        print(
            "ERROR: NCOS device is not in DEV Mode! Unable to install the app into {}.".format(
                g_dev_client_ip
            )
        )
...
def start():
    if is_NCOS_device_in_DEV_mode():
        print(
            "Start application {} for NCOS device at {}".format(
                g_app_name, g_dev_client_ip
            )
        )
        ...
        print(
            "ERROR: NCOS device is not in DEV Mode! Unable to start the app from {}.".format(
                g_dev_client_ip
            )
        )
...
def stop():
    if is_NCOS_device_in_DEV_mode():
        print(
            "Stop application {} for NCOS device at {}".format(
                g_app_name, g_dev_client_ip
            )
        )
        print("Application UUID is {}.".format(g_app_uuid))
        ...
        print(
            "ERROR: NCOS device is not in DEV Mode! Unable to stop the app from {}.".format(
                g_dev_client_ip
            )
        )
...
def uninstall():
    if is_NCOS_device_in_DEV_mode():
        print(
            "Uninstall application {} for NCOS device at {}".format(
                g_app_name, g_dev_client_ip
            )
        )
        print("Application UUID is {}.".format(g_app_uuid))
        ...
        print(
            "ERROR: NCOS device is not in DEV Mode! Unable to uninstall the app from {}.".format(
                g_dev_client_ip
            )
        )
...
def purge():
    if is_NCOS_device_in_DEV_mode():
        print(
            "Purged application {} for NCOS device at {}".format(
                g_app_name, g_dev_client_ip
            )
        )
        print("Application UUID is {}.".format(g_app_uuid))
        ...
        print(
            "ERROR: NCOS device is not in DEV Mode! Unable to purge the app from {}.".format(
                g_dev_client_ip
            )
        )
...
def output_help():
    ...
    print(
        "status: Fetch and print current app status from the locally connected NCOS device.\n"
    )
        ...
        if g_app_name in config:
            if uuid_key in config[g_app_name]:
                ...
                if ceate_new_uuid or g_app_uuid == "":
                    ...
                    print(
                        "INFO: Created and saved uuid {} in {}".format(
                            _uuid, app_config_file
                        )
                    )
            else:
                print(
                    "ERROR: The uuid key does not exist in {}".format(app_config_file)
                )
        else:
            print(
                "ERROR: The APP_NAME section does not exist in {}".format(
                    app_config_file
                )
            )
...
def init(ceate_new_uuid):
    ...
    if sdk_key in config:
        if app_key in config[sdk_key]:
            ...
            success = False
            print(
                "ERROR 1: The {} key does not exist in {}".format(
                    app_key, settings_file
                )
            )
        ...
        if ip_key in config[sdk_key]:
            ...
            success = False
            print(
                "ERROR 2: The {} key does not exist in {}".format(ip_key, settings_file)
            )
        ...
        if username_key in config[sdk_key]:
            ...
            success = False
            print(
                "ERROR 3: The {} key does not exist in {}".format(
                    username_key, settings_file
                )
            )
        ...
        if password_key in config[sdk_key]:
            ...
            success = False
            print(
                "ERROR 4: The {} key does not exist in {}".format(
                    password_key, settings_file
                )
            )
    else:
        success = False
        print(
            "ERROR 5: The {} section does not exist in {}".format(
