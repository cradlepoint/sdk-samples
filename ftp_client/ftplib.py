...
class FTP:
    ...
    def getwelcome(self):
        ...
        if self.debugging:
            ...
            cp.log("FTP", "{} {}".format("*welcome*", self.sanitize(self.welcome)))
        return self.welcome
    ...
    def putline(self, line):
        ...
        if self.debugging > 1:
            ...
            cp.log("FTP", "{} {}".format("*put*", self.sanitize(line)))
    ...
    def putcmd(self, line):
        if self.debugging:
            ...
            cp.log("FTP", "{} {}".format("*cmd*", self.sanitize(line)))
        self.putline(line)
    ...
    def getline(self):
        ...
        if self.debugging > 1:
            ...
            cp.log("FTP", "{} {}".format("*get*", self.sanitize(line)))
        if not line:
            raise EOFError
        if line[-2:] == CRLF:
            line = line[:-2]
    ...
    def getresp(self):
        ...
        if self.debugging:
            ...
            cp.log("FTP", "{} {}".format("*resp*", self.sanitize(resp)))
    ...
    def abort(self):
        ...
        if self.debugging > 1:
            ...
            cp.log("FTP", "{} {}".format("*put urgent*", self.sanitize(line)))
    ...
    def makeport(self):
        ...
        for res in socket.getaddrinfo(
            None, 0, self.af, socket.SOCK_STREAM, 0, socket.AI_PASSIVE
        ):
        ...
        with self.transfercmd(cmd) as conn, conn.makefile(
            ...
            while 1:
                ...
                if self.debugging > 2:
                    ...
                    cp.log("FTP", "{} {}".format("*retr*", self.sanitize(line)))
                if not line:
                    break
                if line[-2:] == CRLF:
                    line = line[:-2]
        ...
        def __init__(
            ...
            if context is None:
                context = ssl._create_stdlib_context(
                    self.ssl_version, certfile=certfile, keyfile=keyfile
                )
