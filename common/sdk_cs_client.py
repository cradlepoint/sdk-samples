...
# logger = logging.getLogger(CSCLIENT_NAME)
# logger.setLevel(logging.INFO)
# log_enable = (os.environ.get('SYSLOG_ENABLE', 'true').lower() == 'true')
# if log_enable:
#     handler = logging.handlers.SysLogHandler(address='/dev/log')
# else:
#     handler = logging.FileHandler('/var/log/{}.log'.format(CSCLIENT_NAME))
...
class CSClient(object):
    ...
    #     except ValueError:
    #         logger.info('Unable to set log level %s', level)
    ...
    def _safe_receive(self, sock):
        ...
        try:
            # logger.debug("body decoded: %s", body)
            ...
            result = body.strip()
        # logger.debug("safe_receive result=\"%s\"", result)
...
class EventingCSClient(CSClient):
    ...
    def start(self):
        if sys.platform == 'linux2':
            if self.running:
                msg = "Eventing Config Store (%s) already running", self.pid
                # logger.debug(msg)
                self.log(CSCLIENT_NAME, msg)
                return
            self.running = True
            self.pid = os.getpid()
            ...
            self.log(CSCLIENT_NAME, 'EventingCSClient.start() is only available when running in the router.')
    ...
    def stop(self):
        if sys.platform == 'linux2':
            ...
            self.log(CSCLIENT_NAME, 'EventingCSClient.stop() is only available when running in the router.')

    def _handle_events(self):
        poller = select.poll()
        poller.register(self.event_sock,
                        select.POLLIN | select.POLLERR | select.POLLHUP)  # I don't unregister this in cleaning up!
        while self.running:
            try:
                events = poller.poll(1000)
                for f, ev in events:
                    if ev & (select.POLLERR | select.POLLHUP):
                        msg = "Hangup/error received. Stopping"
                        # logger.error(msg)
                        self.log(CSCLIENT_NAME, msg)
                    ...
                    if ev & select.POLLIN:
                        ...
                        try:
                            ...
                            errmsg = "Could not find register data for eid %s", (eid)
                            # logger.warning(errmsg)
                            self.log(CSCLIENT_NAME, errmsg)
            except OSError as e:
                errmsg = "OSError: %s", e
    ...
    def register(self, action, path, callback, *args):
        if sys.platform == 'linux2':
            if not self.running:
                raise SdkCSException("Eventing Config Store not started")
            eid = self.eids
            self.eids += 1
            ...
            self.log(CSCLIENT_NAME, 'Event register is only available when running in the router.')
    ...
    def unregister(self, eid):
        if sys.platform == 'linux2':
            ...
            self.log(CSCLIENT_NAME, 'Event unregister is only available when running in the router.')
