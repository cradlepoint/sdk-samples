...
class TestApp(unittest.TestCase):
    ...
    def test_running(self):
        # assume basic first
        res = requests.get(
            "http://{}/api/status/log".format(self.DEV_CLIENT_IP),
        ...
        if "authentication failure" in res.text:
            res = requests.get(
                ...
                auth=requests.auth.HTTPDigestAuth(
                    self.DEV_CLIENT_USER, self.DEV_CLIENT_PASS
                ),
            )
        ...
        self.assertIn(
            "Published msg received. topic: /status/wan/connection_state", res.text
        )
        self.assertIn(
            "Published msg received. topic: /status/system/modem_temperature", res.text
        )
