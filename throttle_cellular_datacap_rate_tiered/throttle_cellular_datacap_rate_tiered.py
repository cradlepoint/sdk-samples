...
class DataUsageCheck(object):
    ...
    def find_modem_profiles(self):
        ...
        modem_profiles_list = [
            x["_id_"] for x in wan_ifcs if x["trigger_string"].startswith("type|is|mdm")
        ]
    ...
    def reset_throttle(self, modem_profiles_list, monthlyreset):
        for mdm in modem_profiles_list:
            if monthlyreset:
                self.cp.delete(self.CFG_RULES2_PATH + "/" + mdm + "/bandwidth_egress")
                self.cp.delete(self.CFG_RULES2_PATH + "/" + mdm + "/bandwidth_ingress")
            else:
                if "bandwidth_egress" in self.cp.get(self.CFG_RULES2_PATH + "/" + mdm):
                    self.cp.delete(
                        self.CFG_RULES2_PATH + "/" + mdm + "/bandwidth_egress"
                    )
        ...
        if monthlyreset:
            self.cp.log("Monthly data usage reset - disabling reduced LTE data rate")
            message = (
    ...
    def set_throttle(self, modem_profiles_list, minbwup, minbwdown, tierset):
        for mdm in modem_profiles_list:
            self.cp.put(self.CFG_RULES2_PATH + "/" + mdm + "/bandwidth_egress", minbwup)
            self.cp.put(
                self.CFG_RULES2_PATH + "/" + mdm + "/bandwidth_ingress", minbwdown
            )
        self.cp.put("config/qos/enabled", True)
        self.cp.log(
            "Exceeded monthly data usage threshold - "
            + str(tierset)
    ...
    def run(self):
        ...
        while True:
            if self.cp.get(self.STATUS_DATACAP_PATH + "/completed_alerts/"):
                alerts = self.cp.get(self.STATUS_DATACAP_PATH + "/completed_alerts/")
                limitreached = 0
                tierset = 0
                ...
                if limitreached > 0 and currtierset != tierset:
                    currtierset = tierset
                    self.set_throttle(
                        modem_profiles_list, minbwup, minbwdown, currtierset
                    )
